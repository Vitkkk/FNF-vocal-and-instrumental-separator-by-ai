...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b) {
        ...
        chooseModel.setText("Importar modelo .ONNX (fallback)");
    ...
    private void installBundledModelIfPresent() {
        if (modelFile.exists() && modelFile.length()>1024) return;
        try (InputStream in=getAssets().open("fnf_separator_v31.onnx");
    ...
    private void refreshModelText() {
        modelText.setText(modelFile.exists()?"Modelo local: pronto ("+(modelFile.length()/1024)+" KB)":"Modelo local: não embutido; use o fallback .ONNX");
    }
